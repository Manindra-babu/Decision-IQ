...
setIsSidebarOpen(false); // Auto-hide the sidebar on navigation
...
<div data-theme={theme}
    ...
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden bg-white dark:bg-slate-950">
        <div
            className="absolute top-[-10%] right-[5%] w-[70%] h-[90%] bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-cyan-300/80 dark:from-cyan-900/40 via-blue-500/60 dark:via-blue-900/30 to-transparent rounded-full blur-[120px] animate-float" />
        <div
            className="absolute top-[-5%] right-[-15%] w-[45%] h-[80%] bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-fuchsia-400/70 dark:from-fuchsia-900/40 via-pink-400/50 dark:via-pink-900/30 to-transparent rounded-full blur-[140px] animate-float-reverse" />
        <div
            className="absolute bottom-[-20%] left-[-10%] w-[50%] h-[60%] bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-cyan-100/60 dark:from-slate-900/60 via-blue-50/40 dark:via-slate-800/40 to-transparent rounded-full blur-[100px] animate-float" />
    </div>
    ...
    <style dangerouslySetInnerHTML={{__html: ` @import
        ...
        infinite; transform-origin: top center; } .glass-panel { background: rgba(255, 255, 255, 0.6); backdrop-filter:
        blur(20px); border: 1px solid rgba(255, 255, 255, 0.8); box-shadow: 0 10px 40px -10px rgba(0,0,0,0.08); }
        [data-theme="dark" ] .glass-panel { background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(255, 255, 255,
        0.05); box-shadow: 0 10px 40px -10px rgba(0,0,0,0.4); } `}} />
    ...
    <main className={`relative z-10 pt-24 pb-12 transition-all duration-300 ease-in-out w-full ${ isAuthenticated ?
        (isSidebarOpen ? 'md:ml-64 md:w-[calc(100%-16rem)]' : 'md:ml-20 md:w-[calc(100%-5rem)]' ) : 'ml-0' }`}>
        {currentView === 'landing' &&
        ...
        <Roadmap activeGoalId={activeGoalId} setActiveGoalId={setActiveGoalId} />}
        {currentView === 'saved-paths' &&
        <SavedPaths navigate={navigate} setActiveGoalId={setActiveGoalId} />}
        {currentView === 'profile' &&
        ...
        <DecisionAnalyzer navigate={navigate} setActiveGoalId={setActiveGoalId} />}
        {currentView === 'risk' &&
...
// --- SIDEBAR COMPONENT ---
function Sidebar({ currentView, navigate, isMobileMenuOpen, setIsMobileMenuOpen, isSidebarOpen, setIsSidebarOpen }) {
return (
<>
...
// --- DASHBOARD COMPONENT ---
function Dashboard({ navigate, setActiveGoalId }) {
...
// --- ROADMAP COMPONENT ---
function Roadmap({ activeGoalId, setActiveGoalId }) {
...
const [nodes, setNodes] = useState(ROADMAP_TEMPLATES[activeGoalId] || ROADMAP_TEMPLATES['ai']);

// Sync roadmap when active goal changes globally
useEffect(() => {
setNodes(ROADMAP_TEMPLATES[activeGoalId] || ROADMAP_TEMPLATES['ai']);
}, [activeGoalId]);
...
function SavedPaths({ navigate, setActiveGoalId }) {
const [paths, setPaths] = useState([
{ id: 'ai', title: 'AI Engineer', date: 'Saved on May 8, 2024', icon:
<Bot size={24} />, color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-200' },
{ id: 'ds', title: 'Data Scientist', date: 'Saved on May 7, 2024', icon:
...
<div className="max-w-5xl mx-auto px-4 sm:px-6 mt-12 relative z-10 animate-fade-in-up">
    <div className="text-center mb-12">
        <h1 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white mb-4 uppercase tracking-tighter">
            Saved Paths</h1>
        <p className="text-slate-600 dark:text-slate-400 text-lg font-medium">Manage your stored career trajectories.
        </p>
    </div>
            ...
            <div className="glass-panel p-8 rounded-sm border border-slate-300 dark:border-slate-800">
                ...
                <div className="space-y-4">
                    <NotificationToggle label="Email Alerts" desc="Receive career match updates via email."
                        active={notifications.email} toggle={()=> setNotifications({...notifications, email:
                        !notifications.email})} />
                        <NotificationToggle label="Push Notifications" desc="Get real-time roadmap notifications."
                            active={notifications.push} toggle={()=> setNotifications({...notifications, push:
                            !notifications.push})} />
                            <NotificationToggle label="Weekly Digest" desc="Summary of market trends and skill gaps."
                                active={notifications.weekly} toggle={()=> setNotifications({...notifications, weekly:
...
setIsProcessing(true);
setTimeout(() => { setIsProcessing(false); setIsFlipping(true); setTimeout(() => { setIsLogin(true); setOtpSent(false);
setIsFlipping(false); }, 800); }, 1500);
}
}
};
...
return (
<div
    className="min-h-[calc(100vh-6rem)] flex flex-col items-center justify-start px-4 sm:px-6 relative z-10 pt-4 pb-12 overflow-y-auto">
    <div className="w-full max-w-lg flex flex-col items-center animate-swing -mt-24">
        <div className="flex justify-center w-full h-[140px] sm:h-[200px] -mb-1 z-0 relative">
            <div
                className="w-[2px] h-full bg-gradient-to-b from-transparent via-slate-400 to-[var(--c-primary)] shadow-[0_0_8px_rgba(var(--c-primary-rgb),0.6)]" />
        </div>
        <div className="w-full relative z-50" style={{ perspective: '1200px' }}>
            <div className="glass-panel rounded-sm p-6 sm:p-10 relative z-50 shadow-xl border border-slate-300 dark:border-slate-800"
                ...
                transition: 'transform 0.8s ease-in-out, opacity 0.8s ease-in-out' , opacity: isFlipping ? 0.3 : 1,
                transformStyle: 'preserve-3d' , background: 'rgba(255, 255, 255, 0.95)' }}>
                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 flex justify-center z-20">
                    <div
                        className="w-6 h-6 bg-slate-100 border border-slate-300 border-b-0 rounded-t-sm flex items-center justify-center">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <>
                            ...
                            <div className="animate-fade-in-up delay-100">
                                <div className="flex items-center justify-between mb-1.5"><label
                                        className="block text-[10px] font-black uppercase tracking-widest text-slate-500">Password</label><a
                                        href="#"
                                        className="text-[10px] font-black text-[var(--c-primary)] hover:underline uppercase tracking-wider">Forgot?</a>
                                </div>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center">
                                        <Lock className="h-4 w-4 text-slate-400" />
                                    </div><input required type="password"
                        ...
                        <div className="space-y-4 animate-fade-in-up">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <AuthField label="Full Name" icon={<User className="h-4 w-4 text-slate-400" />}
                                placeholder="Alex Johnson" />
                                <AuthField label="Email Address" icon={<Mail className="h-4 w-4 text-slate-400" />}
                                type="email" placeholder="alex@example.com" />
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <AuthField label="Study Branch" icon={<BookOpen className="h-4 w-4 text-slate-400" />}
                                placeholder="e.g. Computer Science" />
                                <div>
                                    <label
                                        className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1.5">Completion
                                        Year</label>
                                    <div className="relative">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <AuthField label="Password" icon={<Lock className="h-4 w-4 text-slate-400" />}
                                type="password" placeholder="••••••••" />
                                <AuthField label="Confirm Password" icon={<ShieldCheck
                                    className="h-4 w-4 text-slate-400" />} type="password" placeholder="••••••••" />
                            </div>
                        </div>
                        )}
                        {!isLogin && otpSent && (
                        <div
                            className="animate-fade-in-up bg-white p-6 rounded-sm border border-slate-200 shadow-inner">
                            <div className="flex justify-between items-center mb-4"><label
                                    className="block text-[11px] font-black uppercase tracking-widest text-slate-700">Verification
                                    Code</label><button type="button" onClick={()=> setOtpSent(false)}
                                    className="text-[10px] font-black uppercase text-[var(--c-primary)]">Edit</button>
                            </div>
                            <div className="flex justify-center space-x-2 sm:space-x-3">{[1,2,3,4,5,6].map(i => <input
                                    key={i} required maxLength={1}
                                    className="w-10 h-12 text-center text-xl font-black bg-white border border-slate-300 rounded-sm focus:border-[var(--c-primary)]" />)}
                            </div>
                        </div>
                        )}
                        <div className="pt-2 text-center">
                            <span className="text-slate-500 font-semibold text-xs">{isLogin ? "New to the engine? " :
                        ...
                        <button type="submit" disabled={isProcessing}
                            ...
                            </> : <>{isLogin ? 'Sign In' : otpSent ? 'Verify & Register' : 'Send OTP'}<div
                                    className="bg-white text-slate-900 p-1.5 rounded-sm group-hover:bg-[var(--c-primary)] group-hover:text-white transition-all transform group-hover:translate-x-1">
                                    {otpSent ?
...
<div className="animate-fade-in-up">
    ...
    <div className="relative">
        ...
        <input type={type} required
            className="block w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-sm text-slate-900 font-bold text-sm placeholder-slate-400 focus:outline-none focus:border-[var(--c-primary)] transition-colors"
        ...
        <div className="px-4 py-3 bg-white/50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-800">
            <div className="flex flex-wrap gap-2">
                ...
                <button key={i} onClick={()=> handleSend(prompt)}
                    className="text-[9px] font-black uppercase tracking-widest text-[var(--c-primary)] bg-white
                    dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2.5 py-1.5 rounded-sm
                    hover:bg-[var(--c-primary)] hover:text-white transition-all shadow-sm"
                    >
                    {prompt}
                </button>
                ))}
...
function DecisionAnalyzer({ navigate, setActiveGoalId }) {
...
// Convert selection into a goal ID (mocked for roadmap interlinking)
if (setActiveGoalId) setActiveGoalId('ai');
